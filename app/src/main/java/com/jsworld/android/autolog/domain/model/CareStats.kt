...
/**
 * 횟수는 기록 수가 아니라 묶음(하루) 수로 센다 —
 * 세차하면서 왁스·실내를 함께 했다고 "3회"가 되면 사용자가 아는 횟수와 어긋난다.
 */
fun careCounts(careRecords: List<CareRecord>, today: LocalDate): CareCounts {
    ...
    val sessions = buildCareSessions(careRecords)
    return CareCounts(
        monthCount = sessions.count { it.performedAt?.startsWith(monthPrefix) == true },
        yearCount = sessions.count { it.performedAt?.startsWith(yearPrefix) == true },
        yearCost = careRecords
            .filter { it.performedAt?.startsWith(yearPrefix) == true }
            .sumOf { (it.cost ?: 0).toLong() }
    )
}
