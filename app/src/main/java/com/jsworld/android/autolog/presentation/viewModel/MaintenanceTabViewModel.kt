...
class MaintenanceTabViewModel @Inject constructor(
    ...
    private val noHistoryCountMap = mutableMapOf<Long, StateFlow<Int>>()
    ...
    /**
     * 주기는 있는데 기록이 하나도 없는 항목 수 — 상단 배너용.
     * 이런 항목은 0km/오늘 기준 계산이라 홈·리포트·알림의 임박/초과에서
     * 제외되므로, 여기서 첫 기록 입력을 안내한다.
     */
    fun noHistoryCountState(carId: Long): StateFlow<Int> =
        noHistoryCountMap.getOrPut(carId) {
            carMaintenanceRepository.observeMaintenanceOverview(carId)
                .map { list -> list.count { !it.hasHistory } }
